...
func (d *DebugCommand) Help() string {
	return `Usage: metra debug

  Build an archive containing Metra pprof traces
...
func (d *DebugCommand) Synopsis() string {
	return "Build an archive containing Metra pprof traces"
}
...
func (d *DebugCommand) Run(args []string) int {
	...
	stamped := "metra-debug-" + time.Now().UTC().Format("2006-01-02-150405Z")
