...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(MumbaiFlag.Name) || ctx.GlobalBool(MetraMainnetFlag.Name) {
			homeDir, _ := os.UserHomeDir()
			return filepath.Join(homeDir, "/.metra/data")
		}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(MetraMainnetFlag.Name):
		urls = params.MetraMainnetBootnodes
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(MumbaiFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		homeDir, _ := os.UserHomeDir()
		cfg.DataDir = filepath.Join(homeDir, "/.metra/data")
	case ctx.GlobalBool(MetraMainnetFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		homeDir, _ := os.UserHomeDir()
		cfg.DataDir = filepath.Join(homeDir, "/.metra/data")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, RopstenFlag, RinkebyFlag, GoerliFlag, MumbaiFlag, MetraMainnetFlag, SepoliaFlag, KilnFlag)
	...
	case ctx.GlobalBool(MetraMainnetFlag.Name):
		if !ctx.GlobalIsSet(MetraMainnetFlag.Name) {
			cfg.NetworkId = 191
		}
		cfg.Genesis = core.DefaultMetraMainnetGenesisBlock()
	case ctx.GlobalBool(KilnFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(MetraMainnetFlag.Name):
		genesis = core.DefaultMetraMainnetGenesisBlock()
	case ctx.GlobalBool(KilnFlag.Name):
