...
func remoteConsole(ctx *cli.Context) error {
	...
	if endpoint == "" {
		...
		if path != "" {
			...
			} else if ctx.GlobalBool(utils.MumbaiFlag.Name) || ctx.GlobalBool(utils.MetraMainnetFlag.Name) {
				homeDir, _ := os.UserHomeDir()
				path = filepath.Join(homeDir, "/.metra/data")
			} else if ctx.GlobalBool(utils.SepoliaFlag.Name) {
				path = filepath.Join(path, "sepolia")
			}
		}
		endpoint = fmt.Sprintf("%s/metra.ipc", path)
	}
